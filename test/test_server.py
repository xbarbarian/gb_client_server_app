import socket
import json
from server_v2 import create_server, handle_data, presence, msg
...
def test_presence():
    assert presence({'action': 'presence', 'user': {'account_name': 'Pit'}}) == json.dumps(
        {'response': '200', 'alert': 'Успешно'}, ensure_ascii=False)
